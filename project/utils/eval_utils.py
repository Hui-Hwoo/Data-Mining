...
def fetch_true_answer(data, idx):
...
def evaluate_single_instruction(
    ...
    true_answers = [fetch_true_answer(data, idx=idx) for idx in eval_index_all]
    ...
    prediction_treat_as_number_list = [True] * len(true_answers)

    def _parse_prediction(x, is_gpt_model, treat_as_number, num_decimals):
        if is_gpt_model and r"\boxed" in x:
            return re.findall(r"\\boxed{(.*?)}", x)[0]
        else:
            return metrics.get_normalized_prediction(
                x, treat_as_number=treat_as_number, num_decimals=num_decimals
            )

    # pylint: disable=g-long-lambda
    choices = list(
        map(
            lambda x, y: _parse_prediction(x, is_gpt_model, y, prediction_num_decimals),
            raw_answers_to_parse,
